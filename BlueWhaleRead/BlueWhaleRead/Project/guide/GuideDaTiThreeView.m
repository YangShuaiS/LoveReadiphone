...
@implementation GuideDaTiThreeView{
    UIView * downview;
...
- (void)addview{
    ...
    [xt mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.mas_equalTo(ws).with.offset(LENGTH(20));
        make.width.mas_equalTo(LENGTH(124));
        make.height.mas_equalTo(LENGTH(80));
    }];
    
    BaseLabel * subtitle = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(255, 255, 255) LabelFont:TextFont(15) TextAlignment:NSTextAlignmentLeft Text:@"题型共有四种，分别是：单选题、多选题、连线题、排序题\n四种题型的分数一致，其中多选题和连线题的错误率更高，同学们做题的时候一定要认真仔细哦！\n"];
    ...
    [zdl mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self->downview.mas_bottom).with.offset(-LENGTH(10));
