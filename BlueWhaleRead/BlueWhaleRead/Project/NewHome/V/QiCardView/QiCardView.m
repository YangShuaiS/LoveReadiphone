...
@property (nonatomic, assign) NSInteger nowinters;//!< 当前索引(已显示的最大索引)
...
- (void)reloadDataAnimated:(BOOL)animated {
    ...
    for (NSInteger i = _NowLocation; i < showNumber+_NowLocation; i++) {
...
- (void)createCardViewCellWithIndex:(NSInteger)index {
    ...
    CGFloat yOffset = -((self.interitemSpacing / minHScale) * 1.5 *showCount);
...
//
    for (NSInteger i = 0; i < count; i ++) {
        ...
        CGFloat y = -((self.interitemSpacing / hScale) * 1.5 * showIndex);
...
- (void)cardViewCellDidLastCellFromSuperView:(QiCardViewCell *)cell{
    if (_nowinters > 0) {
        _nowinters--;
        _NowLocation = _nowinters;
