...
- (CGFloat)columnMargin{
    if ([self.delegate respondsToSelector:@selector(columnMarginInWaterFallLayout:)]) {
        ...
        return _LMHDefaultColunmMargin;
    }
}
...
- (void)prepareLayout{
    ...
    // 设置每一列默认的高度
    for (NSInteger i = 0; i < _LMHDefaultColunmCount ; i ++) {
        [self.columnHeights addObject:@(_LMHDefaultEdgeInsets.top)];
    }
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    for (int i = 1; i < _LMHDefaultColunmCount; i++) {
