...
- (void)addview{
    ...
    textimag.backgroundColor = RANDOMCOLOR;
    [self addSubview:textimag];
    [textimag mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self->imageview.mas_bottom).with.offset(LENGTH(4));
        make.left.mas_equalTo(ws);
...
- (void)setNeirong:(ZhiShiShuNeiRongModel *)neirong{
    _neirong = neirong;
    NSString * imageurl = [NSString stringWithFormat:@"http://192.168.1.102/knowledge/public/%@",neirong.pic];
    [self->imageview sd_setImageWithURL:[NSURL URLWithString:imageurl]];
    [self->imageview mas_updateConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(neirong.width*self->poinw);
        make.height.mas_equalTo(neirong.height*self->poinw);
    }];
    [self->textimag sd_setImageWithURL:[NSURL URLWithString:imageurl]];
    label.text = neirong.name;
//    label.textColor = [BaseObject colorWithHexString:@"#6cabdf"];
}
//- (void)addimage{
