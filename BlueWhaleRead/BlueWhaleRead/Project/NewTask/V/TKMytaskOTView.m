...
- (void)addview{
    ...
    time = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(153,153,153) LabelFont:TextFont(13) TextAlignment:NSTextAlignmentLeft Text:@"2018.7.21 - 8.21   共4周"];
    ...
    name = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(51,51,51) LabelFont:TextFontCu(19) TextAlignment:NSTextAlignmentLeft Text:@""];
    [backview addSubview:name];
    [name mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self->backview).with.offset(LENGTH(25));
        make.top.mas_equalTo(self->leftimage.mas_bottom).with.offset(LENGTH(10));
    }];
...
//    time.text = [NSString stringWithFormat:@"%@ - %@  共%ld周",model.studentMission.receive_time,model.studentMission.completed_time,ceilA];
    ...
    name.text = model.mission.mission_name;
